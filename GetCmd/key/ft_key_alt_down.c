...
/*   Updated: 2015/04/05 06:34:34 by bsautron         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_key_alt_down(int *pos)
{
	...
	i = 0;
	if (((ft_lstl_len(g_env.cmd) + ft_strlen(g_env.prompt)) % (g_env.win_col - 1) < (ft_lstl_len(g_env.cmd) - *pos + ft_strlen(g_env.prompt)) % (g_env.win_col - 1) && *pos < g_env.win_col) || *pos > g_env.win_col - 2)
	{
